...
exports.ListCompletedServicesUseCase = void 0;
...
class ListCompletedServicesUseCase {
    constructor(repository) {
        this.repository = repository;
    }
    async execute(filter) {
        if (!filter.range.from || !filter.range.to) {
            throw new http_error_1.HttpError("Range of dates is required", 400);
        }
        const filterDate = {
            from: (0, date_fns_1.add)(new Date(filter.range?.from), { hours: 3 }),
            to: (0, date_fns_1.add)(new Date(filter.range?.to), { hours: 26, minutes: 58 }),
        };
        const valid = ListCompletedServicesUseCase.validateDates(filterDate);
        if (!valid) {
            throw new http_error_1.HttpError("Invalid dates", 400);
        }
        ...
        const resultWithType = resultMalFormatted.map(ListCompletedServicesUseCase.mapTypeService);
        const result = [];
        Object.keys(mappedMetrics).forEach((type) => {
            const summary = mappedMetrics[type].values.reduce((a, v) => ({ ...a, [`até ${v} ${mappedMetrics[type].type}`]: 0 }), { late: 0, total: 0 });
            let collection = {
                tableName: type,
                summary,
                values: [],
    ...
    static validateDates(range) {
        if ((0, date_fns_1.isBefore)(range.to, range.from)) {
            return false;
        }
        if ((0, date_fns_1.differenceInDays)(range.to, range.from) >= 32) {
            return false;
        }
        return true;
    }
    ...
    static mapTypeService(data) {
        ...
        return {
            ...data,
            start_date: (0, date_fns_tz_1.format)(data.start_date, "yyyy-MM-dd HH:mm:ss", {
                timeZone: "America/Sao_Paulo",
            }),
            finish_date: (0, date_fns_tz_1.format)(data.finish_date, "yyyy-MM-dd HH:mm:ss", {
                timeZone: "America/Sao_Paulo",
            }),
            type,
        };
    }
    static updateSummary(dataArray) {
        dataArray.forEach((data) => {
            data.values.forEach((value, index) => {
                let classification = "";
                const startDate = (0, date_fns_1.parseISO)(value.start_date);
                const finishDate = (0, date_fns_1.parseISO)(value.finish_date);
                if (data.tableName === "ARSESP - AGUA") {
                    const duration = (0, date_fns_1.differenceInHours)(finishDate, startDate);
                    if (duration > 96) {
                        data.summary["late"] += 1;
                        classification = "late";
                    }
                    ...
                    const duration = (0, date_fns_1.differenceInDays)(finishDate, startDate);
                    if (duration > 20) {
                        data.summary["late"] += 1;
                        classification = "late";
                    }
...
exports.ListCompletedServicesUseCase = ListCompletedServicesUseCase;
//# sourceMappingURL=list-services.usecase.js.map